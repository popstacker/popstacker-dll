...
private:
	...
	static HRESULT presentCallback(IDXGISwapChain3* swapChain, UINT syncInterval, UINT flags) {
		...
		// Initial and Update DeltaTime
		{
			...
			if (initDeltaTime)
				Direct2D::deltaTime = std::chrono::duration_cast<std::chrono::milliseconds>(currentTime - lastTime).count() / 1000.0f;
		...
		if (SUCCEEDED(swapChain->GetDevice(IID_PPV_ARGS(&d3d12Device)))) {
			...
			Direct2D::NewFrame(swapChain, d3d11Device, (float)GetDpiForWindow(window));
			Direct2D::Render();
			Direct2D::EndFrame();
